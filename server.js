...
const borrarTabla = ()=>{
    conBD.query('DROP TABLE IF EXISTS xyz;', 
...
const crearRegistro = () =>{
    conBD.query('INSERT INTO carros (marca, modelo, year, color, puertas) VALUES (?, ?, ?, ?, ?);', ['Ford','Camioneta','2022','gris','4'],
        function (err, results, fields) {
            if (err) throw err;
            else console.log('Registros insertados ' + results.affectedRows + ' row(s).');
        });
}
...
const actualizarRegistros = ()=>{
    conBD.query('UPDATE carros SET marca = ? WHERE modelo = ?', ['Toyota', 'Maru'], 
            function (err, results, fields) {
                if (err) throw err;
...
const BorrarRegistro = ()=>{
    conBD.query('DELETE FROM carros WHERE id = ?', ['3'], 
    function (err, results, fields) {
        if (err) throw err;
